...
async def main():
    """
    Entry point for the CLI tool to build JAX wheel packages or update
    requirements_lock.txt files. Parses command-line arguments and executes
    the corresponding subcommand logic.
    """
    ...
    with open(".jax_configure.bazelrc", "w", encoding="utf-8") as f:
        ...
        if not jax_configure_options:
            logging.error(
                "Error retrieving the Bazel options to be written to "
                ".jax_configure.bazelrc, exiting."
            )
            sys.exit(1)
    ...
    if args.configure_only:
        ...
        for wheel in args.wheels.split(","):
            ...
            if wheel not in WHEEL_BUILD_TARGET_DICT:
                logging.error(
